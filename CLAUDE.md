...
| exp_007_concept_carving | In progress | Prototype vs. episodic dissociation. Can a learner acquire concepts whose extension does NOT coincide with a native DINOv2 cluster? 6–10 learners × concept types, full stats (CIs, directional paired bootstrap, Wilcoxon). **007a NATIVE: PASS (H2 — all learners coincide).** **007b DISJUNCTIVE: H1 NOT supported** — episodic/multi-anchor do not beat the centroid on cross-cutting unions (1/50 cells); all similarity learners collapse 0.92→0.46. Frozen DINOv2 is a hard *acquisition* ceiling, not just an alignment attractor → argues for trainable perception (GLA). 007c (cross-cutting) pending. |
| exp_007b_extension | PASS | C2 Robustness & Disjunctive Gradient. 3 tiers (near/mid/far) × 6 pool types × 10 learners × 30 episodes. **C1 GRADIENT CONFIRMED** (monotonic): native≈0.97 → near=0.847 → mid=0.654 → far=0.529. **C2 EPISODIC ADVANTAGE ROBUST**: gap ≈ +0.07 in near (p=2e-5) and +0.07 in mid (p<1e-5), stable across all 6 pool constructions. The episodic advantage is not a pool artifact and operates throughout the disjunctive regime. Frozen DINOv2 ceiling is now mapped with gradient shape. |
| exp_007b_extension_clean | PASS | Homogeneous re-run: all 4 tiers × 7 pool types × 6 learners × 30 episodes. **C1 gradient**: native=0.943 → near=0.847 → mid=0.654 → far=0.530 (monotonic, all learners coincide). **C2 gap IS disjunctive-specific** (hard pool): near gap=+0.074 and mid gap=+0.066 are both significantly larger than native gap=+0.032 (unpaired bootstrap CI excludes zero). Far gap=+0.048 not significant vs native. LogReg/LinSVM outperform both centroid and exemplar under hard pools. **NMI_native audit**: 0.357 is a mathematical constant for any 2-from-10 balanced union — gate reduces to imd + NS_silhouette only. |
| exp_010_test2_paper | Pending | Test 2 (second-language acquisition), full paper. Long-horizon — renumbered from exp_007 so Concept Carving (the experiment that makes the paper submittable) takes the next slot. |
