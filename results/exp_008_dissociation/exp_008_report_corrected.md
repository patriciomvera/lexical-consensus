...
- Interpretation: Very low collinearity (|r|=0.155). Adequate dissociation power expected.
...
- R² = 0.2513, VIF_perc = 1.0105, VIF_sem = 1.0105
...
**VIF correction (2026-06-09):** The original run reported VIF = 9.6930 for both
predictors. This was caused by passing the predictor matrix without the intercept
column to `statsmodels.stats.outliers_influence.variance_inflation_factor`, which
forces the auxiliary regression through the origin and artificially inflates R²
when predictor means are far from zero. The corrected computation passes
`sm.add_constant(X)` and yields VIF = 1.0105, consistent with the Pearson
r = 0.1019 between d_perc and d_sem on the sampled pairs (VIF = 1/(1−r²) =
1/(1−0.0104) = 1.0105). No regression coefficients, p-values, R², partial R²,
or decision rule outcomes are affected — only the VIF diagnostic value changes.
The corrected VIF confirms negligible collinearity between the two predictors,
which strengthens rather than weakens Rule A.
