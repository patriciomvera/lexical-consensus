    # Experiment 001b — Condition 2 (HARD)

    **Date:** 2026-05-25
    **Agent:** exp_001b — 10 seeds per category
    **Label assignment:** frog → slithy | horse → mimsy | ship → vorpal
    **Level:** HARD
    **Distractors:** the 2 images from wrong trained categories with HIGHEST cosine similarity to the target centroid (hard negatives, pre-ranked per centroid)
...
| ship  | vorpal   | 1.0000 | PASS |
    ...
    | Metric | Value |
    |---|---|
    | Mean retrieval margin (sim_correct − sim_best_distractor) | 0.3730 |
    | Mean entropy over candidates H(image\|label) | 1.5570 bits |
...
**Hard-negative error rate:** 0.0000
...
target — the boundary most likely to fail in exp_002.
    ...
    *(no failures)*
    ...
    The agent successfully retrieves correct images under distractor pressure at the hard difficulty level. Carroll labels are grounded in the centroid structure, not just memorised naming associations.
